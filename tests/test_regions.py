...
def test_retired_url_serializers_region_gets_an_actionable_error() -> None:
    ...
    A file generated by 0.9.0-0.12.0 with --enrich-url-types still has the
    markers (0.8.0 promoted AnyUrl unconditionally, but both the flag and the
    serializer region arrived in 0.9.0). CHANGELOG tells maintainers to delete
    the markers *or* pass --reset, so the message is pinned in full here —
    matching a fragment would still pass if either recovery path silently
    ...
    assert str(exc.value) == (
        ...
        "generator: aqueduct:generated:url_serializers. "
